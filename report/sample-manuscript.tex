...
\begin{figure}[ht]
    \centering
    \fbox{\parbox[c][0.25\textwidth][c]{0.3\textwidth}{\centering ACO network figure placeholder}}
    \caption{Transit network generated by the ACO algorithm.}
    \label{fig:aco_network}
    \Description{Placeholder for the ACO network map.}
\end{figure}
...
\begin{table*}[h]
\caption{Basic Coverage and Access Metrics}
...
\midrule
Naive & 12.92 & 4.41 & 10010 & 1761 & 0.0189 \\
Iterative & 18.57 & 7.99 & 18009 & 914 & 0.0279 \\
ACO & 32.71 & 12.26 & 9720 & 895 & 0.0528 \\
Genetic & 22.87 & 8.95 & 8848 & 845 & 0.0348 \\
WMATA & 10.37 & 3.03 & 15374 & 1611 & 0.0110 \\
\bottomrule
\end{tabular}
}
\end{table*}
...
\midrule
Naive & 0.508 & 2.1 & 7.6 & 23.6 & 10.3 \\
Iterative & 0.469 & 3.1 & 11.1 & 26.0 & 18.5 \\
ACO & 0.494 & 5.5 & 20.4 & 49.4 & 30.2 \\
Genetic & 0.501 & 3.5 & 13.2 & 36.8 & 20.3 \\
WMATA & 0.546 & 1.1 & 4.7 & 14.8 & 6.9 \\
\bottomrule
\end{tabular}
}
\end{table*}
...
\midrule
Naive & 1486.5 & 0.22 & 1.04 & 14.3 & 67.6 & 18.1 \\
Iterative & 1543.1 & 0.31 & 0.56 & 43.5 & 56.5 & 0.0 \\
ACO & 1783.6 & 0.49 & 1.68 & 11.6 & 36.5 & 51.9 \\
Genetic & 12388.6 & 0.05 & 0.97 & 14.1 & 74.6 & 11.3 \\
...
Several patterns are consistent across the metrics. ACO produces the highest coverage at all walk radii and the strongest high-need coverage, but it also carries the highest transfer burden and a long service-track footprint. The iterative network yields the lowest transfer burden and a comparatively strong balance of equity and coverage, while the naive network is modest on both coverage and efficiency. The genetic network achieves good spatial coverage but does so with an extremely long service-track length, driving its efficiency score down by an order of magnitude relative to other variants. WMATA scores lower on walk-radius coverage, which is expected for a real network designed with feeder buses, park-and-ride, and interlined service rather than uniform 500 m walk access; however, it remains the most efficient per kilometer of service track, consistent with its cost-constrained, operationally mature design.

These tables show that algorithmic networks can be tuned to maximize coverage, but real-world feasibility requires balancing coverage with transfer burden and track length. The ACO variant is the most ambitious in spatial reach, while the iterative variant is the most operationally coherent under the transfer metric. Given WMATA's real-world system length and station count~\cite{lit:wmata_wikipedia}, the generated networks should be interpreted as aggressive build-out scenarios rather than direct replacements at current capital scale.

I also perform some potential use-case evaluation by identifying location pairs and calculating the transit time in each network. I calculate the transit times for my network using my Route Planner tool, and I calculate transit times for the real-world network using the WMATA transit planner application. In this report I compare the results obtained for three sample itineraries chosen to demonstrate a variety of transit uses and to address the shortcomings of the existing WMATA network that I raised in the introduction. The three routes presented in this paper are: College Park to DCA Airport (suburb to important hub), downtown Bethesda to Reston Center (suburb to suburb), and Congress Heights to downtown Silver Spring (disadvantaged neighborhood to suburb). For all three routes, the algorithms developed in this project provide faster transit times than the real-world network, with each algorithm having stronger performance on different routes. The ACO variant is not included in Table~\ref{tab:transittimes} because its route-planner runs were not recorded for this set of examples. Table \ref{tab:transittimes} shows the travel times calculated for each network. 
...
Visual comparison of the generated networks provides additional justification and insight. The naive network shows some suburb-to-suburb connectivity, but it leaves large underserved areas, consistent with its lower point and neighborhood coverage metrics.

The iterative improvement algorithm takes the most time to execute, but produces a more coherent network with strong urban-core connectivity and the lowest transfer burden. Its coverage is moderate, suggesting that the objective function favors cohesive corridors over maximal spatial reach.

The ACO network is the most expansive spatially, with the strongest coverage and high-need access. That coverage comes at the cost of a long service-track length and a high share of 2+ transfer pairs, indicating a network that is ambitious in reach but operationally complex.

The genetic algorithm produces strong core-to-suburb connections and comparatively high neighborhood coverage, but its service-track length is extreme, which drives down efficiency. This likely reflects the service-pattern reward weight, which favors extensive reach at the expense of construction realism.
