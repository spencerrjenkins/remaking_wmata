...
Employment origin-destination data from the LEHD LODES8 program~\cite{lodes2023} is downloaded for DC, MD, and VA, aggregated from census blocks to tracts, and combined into a demand surface based on job-flow volumes to the top origin-destination pairs. This commute-pattern-aware signal ensures that major employment clusters---the Pentagon, NIH, Tyson's Corner---receive appropriate demand weighting independent of residential population density.
...
\]
where $O_i$ and $D_j$ are origin and destination demand signals combining population density and LODES employment OD flows~\cite{lodes2023}, and $d_{ij}$ is network distance. The model is calibrated to WMATA's FY2023 ridership of approximately 350,000 daily boardings~\cite{lit:wmata_stats}. Daily totals are disaggregated by time of day using WMATA's observed ridership profile: AM peak 26\%, PM peak 27\%, midday 30\%, evening 10\%, late night 5\%, and early morning 2\%. Station-level boardings and hourly profiles are stored in the output GeoJSON and visualized in the Transit Network Viewer as interactive sparkline charts.
